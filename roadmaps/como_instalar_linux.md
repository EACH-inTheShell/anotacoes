...
Primeiro vamos começar com as opções, não existe apenas essas opções para usar linux mas recomendamos essas opções para user durante as atividades em grupo! Uma VM (Máquina Virtual) ou WSL podem dificultar durante as atividades do grupo, já que algumas ferramentas usam bastante memória e podem ser bem pesadas e também vamos encontrar alguns protocolos que vão precisar de GUI por exemplo o rdp (não queremos focar muito na parte de fazer um WSL funcionar com rdp por exemplo, pois vai atrasar as atividades da semana).
